Shader "Custom/Texture3DLitSkyExtColorAlignNoise"
{
    Properties
    {
        _MainTex("Texture", 3D) = "" {}
    ...
    SubShader
    {
        ...
        float4 frag(v2f IN) : COLOR
        {
            ...
            float3 SD = float3(8.0, 16.0, 32.0);
            ...
            float4 planeAlignSetup = RayMarchCubeSetup(Steps, IN.localPos, _LocalBoundsMaximum);

            // Unreal setup - from other nodes
            int MaxSteps = clamp(floor(Steps * planeAlignSetup.a), 0, 512);
            float FinalStepSize = frac(Steps * planeAlignSetup.a);
            float3 localCameraPosition = UNITY_MATRIX_IT_MV[3].xyz;
            float3 CurPos = planeAlignSetup.xyz;// IN.color;
            ...
            for (int i = 0; i < MaxSteps; i++)
            {
                ...
                float fractalNoise = Distort(_NoiseVolume, sampler_NoiseVolume, CurPos);
                CurPos += fractalNoise;
